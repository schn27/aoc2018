...
function calc() {
	...
	input.split("\n").sort().map(e => e.split(/[\s-\]:]/g)).forEach(e => {
		switch (e[6]) {
		case "Guard":
			id = +e[7].substring(1);
			break;
		...
		case "wakes":
			let wakes = +e[4];
			let timeTable = guards[id] || [];

			for (let m = falls; m < wakes; ++m) {
				timeTable[m] = (timeTable[m] || 0) + 1;
			}
	...
	const id1 = getKeyOfMaximum(guards, v => Object.values(v).reduce((a, e) => a + e));
	const id2 = getKeyOfMaximum(guards, v => v[getKeyOfMaximum(v)]);

	return id1 * getKeyOfMaximum(guards[id1]) + " " + id2 * getKeyOfMaximum(guards[id2]);
}

function getKeyOfMaximum(o, func) {
	let res = null;
	let max = 0;

	for (let k in o) {
		let v = (func != undefined) ? func(o[k]) : o[k];
		if (v > max) {
			max = v;
			res = k;
		}
...
const input = `[1518-08-12 00:43] falls asleep
