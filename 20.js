...
function calc() {
	const exp = input.split("").filter(c => c != "^" && c != "$");
	...
	let maze = mapToMaze(map);
	let part1 = null;
	let part2 = null;

	for (let wave = 0; part1 == null; ++wave) {
		if (wave == 999 * 2) {
			part2 = maze.reduce((a, r, y) => a + (y & 1) * r.filter((e, x) => e == "." && (x & 1) == 1).length, 0);
		}
...
function parse(exp) {
	...
	for (let i = 0; i < exp.length; ++i) {
		if (exp[i] == "(") {
			...
			let cur = paths[paths.length - 1].slice();
			paths.pop();

			parse(exp.slice(i + 1, j - 1)).forEach(p => {
				const t = cur.slice();
				t.push(...p);
				paths.push(t)
			});
			...
			paths[paths.length - 1].push(exp[i]);
		}
	}
...
function mapToMaze(map) {
	const xmin = Math.min(...[...map].map(k => k.split(",")[0])) - 1;
	const xmax = Math.max(...[...map].map(k => k.split(",")[0])) + 1;
	const ymin = Math.min(...[...map].map(k => k.split(",")[1])) - 1;
	const ymax = Math.max(...[...map].map(k => k.split(",")[1])) + 1;
